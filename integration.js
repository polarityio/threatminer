    ...
    results.forEach((result) => {
      if (
        ...
        lookupResults.push({
          entity: result.entity,
          data: {
            summary: [`Total Results: ${result.body.results.length}`],
            details: result.body
          }
        });
      }
...
function handleRestError(error, entity, res, body) {
  ...
  if (res.statusCode === 200) {
    ...
    // We've seen isntances where the ThreatMiner server returns no `body` and only an HTTP statuscode
    // so we added guards for an undefined `body` parameter.
    result = {
      error: body ? body : 'Unknown Error',
      detail:
        body && body.error && body.message
          ? `${body.error}: ${body.message}`
          : `Unexpected HTTP Status Code ${res.statusCode} received.`
    };
  }
  return result;
}
